//
//  ContentViewArchive.swift
//  VisionOS Course Trail 03
//
//  Created by ClareZhou on 2024/9/4.
//
...
/*
 ...
 struct MultipleBallsView: View {
         ...
         if balls.children.isEmpty {
             for _ in 0..<ballTotal {
                 ...
                 ball.physicsBody = PhysicsBodyComponent(
                     massProperties: PhysicsMassProperties(shape: .generateSphere(radius: 0.04), mass: 1),
                     material: PhysicsMaterialResource.generate(staticFriction: 0.5, dynamicFriction: 0.5, restitution: 0.01),
                     mode: .dynamic
                 )
                 ball.collision = CollisionComponent(shapes: [ShapeResource.generateSphere(radius: 0.04)])
     ...
     private func dropNextBall() {
         ...
         // 将小球位置设置为固定位置
         ball.position = [0, 0.5, 0] // 设置生成在相同位置
         ...
         balls.addChild(ball) // 直接将 ball 添加到 balls 容器中
